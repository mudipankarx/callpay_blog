...
$query = "SELECT * FROM `wp_posts` where `post_title` like '%$search_val%' and `post_type`='post'";
...
<div class="container-fluid">
	   ...
	   ?>     
              ...
              <p>
                <?php echo wp_trim_words($post->post_content,40); ?>
              </p>
              <button><a href="<?php echo esc_url( get_permalink($post->ID));?>">Read More</a></button>     
              <hr>
	<?php } if($a<=1){?>
	    <div style="padding:20px; color:red; font-size:20px; font-weight:600; border:1px dotted #ccc; text-align:center; min-height:100%">No Matching Record Found</div>
	<?php }
			   }else{	?>
			   <div style="padding:20px; color:red; font-size:20px; font-weight:600; border:1px dotted #ccc; text-align:center; min-height:100%">No Matching Record Found</div>
			   <?php }?>
